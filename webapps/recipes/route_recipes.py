...
from db.repository.recipes import list_recipes, list_recipes_for_current_user
    ...
    if form.is_valid():
        try:
            ...
            form.__dict__.get("errors").append(
                "Создавать рецепты могут только зарегистрированные пользователи."
            )
...
def show_recipes_to_delete(request: Request, db: Session = Depends(get_db)):
    ...
    recipes = list_recipes_for_current_user(db=db, owner_id=current_user_id)
    return templates.TemplateResponse(
