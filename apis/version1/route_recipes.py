...
from db.repository.recipes import create_new_recipe, rate_recipe_by_id
...
from db.repository.users import update_user_rating_by_id
...
def update_recipe(id: int, recipe: RecipeCreate, db: Session = Depends(get_db)):
    ...
    if not message:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Рецепт с id {id} не найден"
        )
    return {"msg": "Успешно обновлено."}

@router.get("/rate/{id}")
def update_recipe(id: int, request: Request, db: Session = Depends(get_db), current_user: User = Depends(get_current_user_from_token)):
    rate_recipe_by_id(id=id, db=db, owner_id=current_user.id)
    recipe = retreive_recipe(id=id, db=db)
    message = update_user_rating_by_id(id=recipe.owner_id, db=db)
    if not message:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Пользователь с id {id} не найден"
        )
    referer_url = request.headers.get("referer")
    if referer_url:
        return Response(status_code=status.HTTP_302_FOUND, headers={"Location": referer_url})
    else:
        # Handle the case where the referer header is missing
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Referer header missing"
        )
...
def delete_recipe(
    ...
    raise HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED, detail="У вас нет прав!!!!"
    )
